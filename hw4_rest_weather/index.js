...
Object.keys(state_info).forEach(function(key) {
    var value_state = state_info[key];
    var currentCity = value_state['capital'];
    var lat = value_state['lat'];
    var lng = value_state['lng'];

    
    var urlName = 'https://api.darksky.net/forecast/46dba78778fa6c187169f606374fb83d/' + lat + ',' + lng;
   $.ajax({url:urlName, dataType:"jsonp"}).then(function(data) {
                console.log(data)
                ...
                else if(TempVal > 10.0 && TempVal <= 30.0){
                    $(document).ready(function() {
                        $(code).css('fill', 'cyan');
                    });
                }
                else if(TempVal > 30.0 && TempVal <= 50.0){
                    $(document).ready(function() {
                        $(code).css('fill', 'green');
                    });
                }
                else if(TempVal > 50.0 && TempVal <= 80.0){
                    $(document).ready(function() {
